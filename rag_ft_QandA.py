...
def pre_train_model(model, tokenizer, test_samples):
    ...
    if isinstance(filtered_baseline_results, dict):
        print(f"Question: {filtered_baseline_results['question']}\nAnswer: {filtered_baseline_results['answer']}\nInference Time: {filtered_baseline_results['inference_time']:.2f} seconds\n")
    else:
...
if __name__ == "__main__":
    ...
    if reranked_docs:
        print("Top re-ranked document example:", reranked_docs[0].page_content if hasattr(reranked_docs[0], 'page_content') else str(reranked_docs[0]))
    ...
    print(f"Number of Q/A pairs in dataset: {len(dataset['train'])}")
